	...
	*c = 0;
	for (i = 0; i < 8; i++) {
		...
		case 0:
			...
			bitSet = isShortBitSet(encodedNum, i+3);
			if(bitSet){
		...
		case 1:
			// check if bit i is set
			bitSet = isShortBitSet(encodedNum, i+4);
			if(bitSet){
				// set the bit at the char c
				setCharBit(i, c);
			}
		...
		case 4:
			// check if bit i is set
			bitSet = isShortBitSet(encodedNum, i+5);
			if(bitSet){
				// set the bit at the char c
				setCharBit(i, c);
			}
...

{
	...
	// check parity bit p1
	parity = isShortBitSet(*num, 1);
    	// get the bits related to P1 using P1_MASK
	mask = P1_MASK;
	sum = *num & mask;
   
    	// count the bits
	numberOfBits = countBits(sum);
	...
	// check parity bit p8
	parity = isShortBitSet(*num, 8);
    	// calculate the parity for P8
	mask = P8_MASK;
	sum = *num & mask;

	numberOfBits = countBits(sum);
	// Checking parity bits for p8
	if(numberOfBits % 2 != 0){
		storedParity = 1;
	}
	else{
